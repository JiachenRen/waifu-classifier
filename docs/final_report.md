...
For the final project, we implemented a method of classifying an anime character as a waifu, husbando or trash character. First, we use a web scraper to collect data about the anime character. Next, we trained an unsupervised learning model (Ngram and Word2Vec) for analyzing the description text. We implemented Ngram first, but due to poor performance, we switched to a Word2Vec implementation. The output of the Word2Vec model is then fed to the trained SVM model (supervised learning), which classifies the gender of the charater with high accuracy (94.4%). Next, we find the popularity rank of the character based on a CNN regression model (with a ranking error of approximately 20%).
Combined with the gender, the rank gives the classification as waifu/husbando/trash. For instance, a good rank combined with a female gender will give the label of waifu and a bad rank would give the classification of trash, irrespective of gender.
